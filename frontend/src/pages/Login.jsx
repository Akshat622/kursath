...
export default function Login() {
  const { login, error, token, loading, loginWithGoogle } = useAuth();
  ...
  const handleGoogleCallback = async (response) => {
    ...
    if (success) {
      // If role is user, go to home page (they are not allowed in admin area)
      navigate('/');
    }
  };
          ...
          <>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-3 border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl text-sm transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
